...
<div id="comments">
	...
	<?php
		include("connect.php");
		$result = mysqli_query($link, "SELECT id, username, recipe, comment, time FROM comments WHERE recipe='pancakes'");
...
<?php
	if($_SERVER['REQUEST_METHOD'] != 'POST') {
		...
		if (!empty($_POST['editid']) && !empty($_POST['comment'])) { // If 'editid' is sent by form, we are editing a comment
			...
			$sql = "INSERT INTO comments (username, recipe, comment) VALUES ('" . $_POST['username'] . "', 'pancakes', '" . $_POST['comment'] . "')";
            $result = mysqli_query($link, $sql);
            if(!$result) {
				$_SESSION["message"] = mysqli_error();
				$_SESSION["success"] = false;
